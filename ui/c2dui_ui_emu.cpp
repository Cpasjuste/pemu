...
void UiEmu::addVideo(C2DUIVideo *v) {
    delete (video);
    video = v;
    video->setShader(pMain->getConfig()->get(ConfigNew::Id::ROM_SHADER, true)->getArrayIndex());
    video->setFilter((Texture::Filter) pMain->getConfig()->get(ConfigNew::Id::ROM_FILTER, true)->getArrayIndex());
    video->updateScaling();
    add(video);
}
...
void UiEmu::onUpdate() {
    ...
    if (!isPaused()) {
        // fps
        bool showFps = pMain->getConfig()->get(ConfigNew::Id::ROM_SHOW_FPS, true)->getInteger();
        if (showFps) {
