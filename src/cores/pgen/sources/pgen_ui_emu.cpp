...
void PGENUiEmu::loadBios() {
    ...
    const auto size = getUi()->getIo()->read(MD_BIOS, (char *) boot_rom, 0x800);
    if (size > 0) {
        /* check BOOT ROM */
...
void PGENUiEmu::loadBram() {
    ...
    if (scd.cartridge.id) {
        // TODO: read chunks
        size = getUi()->getIo()->read(ramPath + CD_BRAM_CART, (char *) scd.cartridge.area, scd.cartridge.mask + 1);
        if (size == scd.cartridge.mask + 1) {
...
void PGENUiEmu::loadSram() {
    if (sram.on) {
        const auto ramPath = pMain->getIo()->getDataPath() + "rams/";
        const auto size = getUi()->getIo()->read(
            ramPath + Utility::removeExt(currentGame.path) + ".srm", (char *) sram.sram, 0x10000);
        if (size < 0x10000) {
