...
void PNESUiEmu::onUpdate() {
    static unsigned int turboCounter = 0;

    if (!isPaused()) {
        // Turbo pulse:
        // timing_turbopulse = 3 means:
        // 3 frames ON, 3 frames OFF.
        // NTSC 60 fps -> about 10 presses/sec.
        // PAL  50 fps -> about 8.3 presses/sec.
        const unsigned int turboPulse =
            conf.timing_turbopulse > 0 ?
            static_cast<unsigned int>(conf.timing_turbopulse) : 3U;

        const bool turboOn =
            ((turboCounter / turboPulse) & 1U) == 0;
        ...
        for (int i = 0; i < NUMGAMEPADS; i++) {
            ...
            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Start) > 0 ?
                Nes::Core::Input::Controllers::Pad::START : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Select) > 0 ?
                Nes::Core::Input::Controllers::Pad::SELECT : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Up) > 0 ?
                Nes::Core::Input::Controllers::Pad::UP : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Down) > 0 ?
                Nes::Core::Input::Controllers::Pad::DOWN : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Left) > 0 ?
                Nes::Core::Input::Controllers::Pad::LEFT : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Right) > 0 ?
                Nes::Core::Input::Controllers::Pad::RIGHT : 0;

#ifdef __3DS__

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::A) > 0 ?
                Nes::Core::Input::Controllers::Pad::A : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::B) > 0 ?
                Nes::Core::Input::Controllers::Pad::B : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::X) > 0 ?
                Nes::Core::Input::Controllers::Pad::A : 0;

            cNstPads->pad[i].buttons |=
                (players[i].buttons & c2d::Input::Button::Y) > 0 ?
                Nes::Core::Input::Controllers::Pad::B : 0;

#else

            // Nintendo Switch / other non-3DS platforms
            //
            // B = NES A
            // A = NES B
            // Y = Turbo A
...
void audio_queue() {
    ...
    if (audio) {
        ...
        if (samples + queued > capacity) {
            printf(
                "WARNING: samples: %i, queued: %i, capacity: %i (fps: %i)\n",
                samples,
                queued,
                capacity,
                nst_pal() ? 50 : 60
...
void audio_set_params(Sound::Output *soundoutput) {
    ...
    if (aud) {
        ...
        sound.SetSampleRate(
            static_cast<unsigned long>(conf.audio_sample_rate)
        );
        ...
        soundoutput->length[0] =
            static_cast<unsigned int>(aud->getSamples());
...
void audio_adj_volume() {
    ...
    sound.SetVolume(
        Sound::CHANNEL_SQUARE1,
        conf.audio_vol_sq1
    );

    sound.SetVolume(
        Sound::CHANNEL_SQUARE2,
        conf.audio_vol_sq2
    );

    sound.SetVolume(
        Sound::CHANNEL_TRIANGLE,
        conf.audio_vol_tri
    );
...
int PNESUiEmu::nestopia_core_init(const char *rom_path) {
    ...
    std::string data_path =
        pMain->getIo()->getDataPath();

    strncpy(
        nstpaths.nstdir,
        data_path.c_str(),
        sizeof(nstpaths.nstdir)
    );

    strncpy(
        nstpaths.nstconfdir,
        data_path.c_str(),
        sizeof(nstpaths.nstconfdir)
    );

    snprintf(
        nstpaths.palettepath,
        sizeof(nstpaths.palettepath),
        "%s%s",
        nstpaths.nstdir,
        "custom.pal"
    );
    ...
    // Initialize and load FDS BIOS,
    // NstDatabase.xml, and custom palette
    nst_fds_bios_load();
    nst_db_load();
    ...
    nst_video_set_dimensions_screen(
...
// These callbacks are retained because other parts of
// the Nestopia frontend expect them to exist.
// Turbo input for Switch is implemented directly
// in PNESUiEmu::onUpdate().
void nst_input_turbo_pulse(
